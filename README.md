...
- All your project code into a single main package (`skilang/`)
...
1. Use this template to create a new GitHub repository, say `skilang`
...
2. Clone the `skilang` repository

3. Open a shell into your local `skilang` directory and run
    ```bash
    ./rename-template.sh skilang
    ```

    This will coherently rename the template's project name with the one chosen by you (i.e. `skilang`, in this example)
...
13. Put your main (resp. test) code in `skilang/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `skilang` package:
```bash
python -m skilang
```

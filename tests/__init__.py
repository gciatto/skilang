...
meaningless_yet_valid_expressions = {
    r"a + b * c - d / e // f % d": Expression(
        "-",
        Expression("+", Symbol("a"), Expression("*", Symbol("b"), Symbol("c"))),
        Expression(
            "%",
            Expression("//", Expression("/", Symbol("d"), Symbol("e")), Symbol("f")),
            Symbol("d"),
        ),
    ),
    r"(a+b)*(c-d)/(e//(f%d))": Expression(
        ...
        Expression("//", Symbol("e"), Expression("%", Symbol("f"), Symbol("d"))),
    ),
    "x.y[0]": Expression("[]", Expression(".", Symbol("x"), Symbol("y")), Term(0)),
    "x.y[0].z": Expression(
        ".",
    ...
    "+x & -y | ~z": Expression(
        "|",
        Expression("&", Expression("+", Symbol("x")), Expression("-", Symbol("y"))),
        Expression("~", Symbol("z")),
    ),
    "f() + g(x) + h(y, z)": Expression(
        "+",
        Expression(
            "+",
            Predicate(
            ...
            Predicate("g", Symbol("x")),
        ),
        Predicate("h", Symbol("y"), Symbol("z")),
    ),
    "x.f(1)": Expression(".", Symbol("x"), Predicate("f", Term(1))),
    "x == y": Expression("==", Symbol("x"), Symbol("y")),
    "x != y": Expression("!=", Symbol("x"), Symbol("y")),
    "x < y": Expression("<", Symbol("x"), Symbol("y")),
    "x <= y": Expression("<=", Symbol("x"), Symbol("y")),
    "x > y": Expression(">", Symbol("x"), Symbol("y")),
